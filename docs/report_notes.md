...
- Metrics: LLM validation rate, average validation score, average score delta, max repair rounds, candidate scores, selected candidate, critic scores, stage status, errors, and OpenRouter usage.
...
2. Idea: convert icon generation into LLM planning, candidate drafting, independent critique, consensus selection, validation, and repair.
3. System diagram: prompt to OpenRouter plan to 3 candidates to semantic/SVG-quality critics to selector to refined SVG.
4. Demo: use the Web UI to show candidate drafts, selected winner, trace, raw LLM responses, and final PNG.
